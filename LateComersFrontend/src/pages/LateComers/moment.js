...
const GetInfo = props => {
  ...
  useEffect(() => {
    ...
    axios.get(baseUrl + "/today-Student-OutData")
      .then(result => {
    ...
    axios.get(baseUrl + "/today-Faculty-InData")
      .then(result => {
  ...
  const handleOutSubmit = e => {
    ...
    if (outSearchParameter.includes("VISITOR")) {
      ...
      axios.put(baseUrl + "/update-Visitor-OutDate", data)
        .then(result => {
          // console.log(result.data);
      ...
      axios.post(baseUrl + "/add-Faculty-Outdata", data)
        .then(result => {
          if (result.status == 201) {
            toast.warn(
